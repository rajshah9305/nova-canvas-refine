  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 md:px-12 lg:px-16">
        <StaggerGroup className="space-y-0" gap={0.07}>
          {/* Top meta strip */}
          <StaggerItem className="flex items-center justify-between gap-3 font-mono text-[9px] sm:text-[10px] md:text-[11px] tracking-[0.25em] sm:tracking-[0.3em] text-foreground/70 mb-8 sm:mb-10 md:mb-14">
          ...
          {/* Editorial label */}
          <StaggerItem className="flex items-center gap-3 mb-4 md:mb-6 flex-wrap">
          ...
          <StaggerItem className="mt-5 sm:mt-6 md:mt-8 grid md:grid-cols-[auto_1fr] items-end gap-x-6 gap-y-2">
          ...
          <StaggerItem className="mt-8 sm:mt-10 md:mt-12 grid md:grid-cols-12 gap-6 md:gap-10 max-w-5xl">
        ...
        {/* Stat ribbon — independent stagger when in view */}
        <StaggerGroup
        ...
        >
          {STATS.map((s, i) => (
            <motion.div
              key={s.v}
              variants={item}
              className={`group p-4 sm:p-5 md:p-6 transition-colors hover:bg-primary/5 ${
